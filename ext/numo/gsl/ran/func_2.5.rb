[{:func_name=>"gsl_cdf_beta_P",
 ...
 {:func_name=>"gsl_ran_choose",
  ...
  :desc=>
   ...
   "only appear once in dest.  It is required that k be less\n" +
 ...
 {:func_name=>"gsl_ran_dir_nd",
  ...
  :desc=>
   ...
   "v = (x_1,x_2,\\ldots,x_n)\n" +
   "in n dimensions.  The vector is normalized such that \n" +
 ...
 {:func_name=>"gsl_ran_wishart",
  ...
  :args=>
   [["const gsl_rng *", "r"],
    ...
    ["gsl_matrix *", "work"]],
  :desc=>
   "This function computes a random symmetric p-by-p matrix from the Wishart distribution.\n" +
   "The probability distribution for Wishart random variates is,\n" +
   ".. only:: not texinfo\n" +
   ".. only:: texinfo\n" +
   "Here, n > p - 1 is the number of degrees of freedom, V is a symmetric positive definite\n" +
   "p-by-p scale matrix, whose Cholesky factor is specified by L, and work is\n" +
   "p-by-p workspace. The p-by-p Wishart distributed matrix X is stored\n" +
   "in result on output."},
 {:func_name=>"gsl_ran_wishart_log_pdf",
  :func_type=>"int",
  :args=>
   [["const gsl_matrix *", "X"],
  ...
  :desc=>
   ...
   "X, whose Cholesky factor is specified in L_X. The degrees of freedom is\n" +
   "given by n, the Cholesky factor of the scale matrix V is specified in L,\n" +
   "and work is p-by-p workspace. The probably density value is returned\n" +
   "in result."},
 {:func_name=>"gsl_ran_wishart_pdf",
  :func_type=>"int",
  :args=>
   [["const gsl_matrix *", "X"],
    ["const gsl_matrix *", "L_X"],
